...
# Tracks which root-level vanity directories were generated by contact pages.
_CONTACT_VANITY_MANIFEST = ".contact_vanity_dirs"
...
def _contact_vanity_manifest_path(output_root):
    """Return absolute path for the contact vanity manifest file."""
    return os.path.join(output_root, _CONTACT_VANITY_MANIFEST)


def _load_contact_vanity_dirs(output_root):
    """Load previously generated vanity directory names from manifest."""
    manifest_path = _contact_vanity_manifest_path(output_root)
    try:
...
def _save_contact_vanity_dirs(output_root, vanity_dirs):
    """Persist the current set of vanity directory names."""
    manifest_path = _contact_vanity_manifest_path(output_root)
    try:
        with open(manifest_path, "w", encoding="utf8") as manifest:
            for vanity_dir in sorted(vanity_dirs):
                manifest.write(f"{vanity_dir}\n")
    except OSError:
        pass


def _discover_contact_vanity_dirs(output_root):
    """Discover legacy vanity directories by contact sort variant files.

    This supports one-time cleanup on repos that predate the vanity manifest.
    """
    if not os.path.isdir(output_root):
        return set()

    vanity_dirs = set()
    sort_variants = [filename for filename in CONTACT_SORT_FILE_MAP.values() if filename != "index.html"]
    for entry in os.listdir(output_root):
        entry_path = os.path.join(output_root, entry)
        if not os.path.isdir(entry_path):
            continue
        if any(os.path.exists(os.path.join(entry_path, variant)) for variant in sort_variants):
            vanity_dirs.add(entry)
    return vanity_dirs


def _collect_current_contact_vanity_dirs(relay_set):
    """Collect safe vanity directory names expected for this run."""
    if not relay_set.base_url:
        return set()

    contact_data_map = relay_set.json.get("sorted", {}).get("contact", {})
    vanity_dirs = set()
    for contact_data in contact_data_map.values():
        aroi_domain = contact_data.get("aroi_domain")
        if aroi_domain and aroi_domain != "none" and contact_data.get("is_validated_aroi"):
            vanity_dirs.add(_sanitize_path_component(str(aroi_domain).lower()))
    return vanity_dirs


def _cleanup_stale_contact_vanity_dirs(output_root, current_vanity_dirs):
    """Remove vanity dirs that existed previously but are not expected now."""
    previous_vanity_dirs = _load_contact_vanity_dirs(output_root)
    if not previous_vanity_dirs:
        previous_vanity_dirs = _discover_contact_vanity_dirs(output_root)

    stale_vanity_dirs = previous_vanity_dirs - set(current_vanity_dirs)
    for stale_dir in stale_vanity_dirs:
        _cleanup_vanity_sort_files(os.path.join(output_root, stale_dir))
...
def write_pages_by_key(relay_set, k):
    ...
    if k == "contact":
        output_root = os.path.dirname(output_path)
        current_vanity_dirs = _collect_current_contact_vanity_dirs(relay_set)
        _cleanup_stale_contact_vanity_dirs(output_root, current_vanity_dirs)
        ...
        _save_contact_vanity_dirs(output_root, current_vanity_dirs)
        return
