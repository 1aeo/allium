...
VOLATILE_RE = re.compile(
    ...
    r'|Last fetch was at \d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}'  # fetch timestamp
    r'|fetched \d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}'  # collector fetched timestamp
    r'|\d+ (?:second|minute|hour|day|week|month|year)s?\s*ago'  # "5 days ago"
    r'|\d+[ywdhms]\s*(?:\d+[ywdhms]\s*)*(?:ago)?'     # "3w 2d 5h 17m ago"
    r'|UP \d+%'                                        # uptime percentage
    r'|\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}'           # bare datetime
...
def classify_file(args):
    ...
    # HTML files: strip volatile data and compare what remains
