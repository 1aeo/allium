...
Live example: `F3Netze` stays near 1:1 read/write around 300–530 Mbit/s. A 5–10× split is the anomaly. New relay `PirateyMatey` (NL, consensus weight 1, two days of history) is the "stuck at weight 1" case. Restart is a point at `last_restarted`. Overload is a current-status badge (Onionoo has no history series; consensus files do not carry overload), not a band on the x-axis.
